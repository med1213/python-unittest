...
class CreateTestPrizeType(APITestCase):
    ...
    def setUp(self):
        self.period_urls = reverse('prize_types')
        self.period_url = reverse('prize_type', kwargs={'pk': 2})
        self.period_data = PrizeType.objects.create(id=2, prize_type='big', is_active=True)
    ...
    def test_get_one(self):
        ...
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(self.period_data.prize_type, "big")
...
class FilterUsercandidateAPITestCase(APITestCase):
    def setUp(self) -> None:
        ...
        self.period_data = PrizeType.objects.create(id=1, prize_type='big', is_active=True)

        self.urls = '{url}?{filter}={value}&{filter1}={value1}&{filter2}={value2}'.format(
            url=reverse('prize_types'),
